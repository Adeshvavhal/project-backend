...
app.get("/employees/teams", async (req, res) => {
  try {
    const employees = await User.find(
      {  role: { $in: ["employee", "Team_Leader"] }}, // ✅ only employees
      {
        employeeId: 1,
        name: 1,
        department: 1,
  ...
  try {
    ...
    if (reportingManagerId) {
      await Notification.create({
        ...
        triggeredByRole: employee.role || "employee",
...
//rutuja api to get TL Team Members
app.get("/:teamLeadId/members", async (req, res) => {
  try {
    ...
    const teams = await Team.find({ teamLead: teamLeadId })
      .populate({
        path: "assignToProject",
        select: "name email employeeId designation contact doj"
      });
    ...
    teams.forEach(team => {
      team.assignToProject.forEach(member => {
        if (member._id.toString() !== teamLeadId && !memberIds.has(member._id.toString())) {
